...
static void event(CmScene *scene, CmEvent *event) {
  ...
  CmCamera3D *camera = &sandbox->camera;
...
static void init(CmScene *scene) {
  ...
  sandbox->mesh = cm_mesh_create(&scene->gpu, ARRAY_LEN(data), data);
  ...
  cm_mesh_attach_ebo(&sandbox->mesh, ARRAY_LEN(cube_indices), cube_indices);
  ...
  cm_mesh_attach_vec3(&sandbox->mesh, ARRAY_LEN(normals), normals);
  ...
  cm_mesh_attach_vec2(&sandbox->mesh, ARRAY_LEN(uv), uv);
  ...
  cm_mesh_attach_mat4_instanced(&sandbox->mesh, ARRAY_LEN(transforms),
                                transforms);

  RGFW_window *window = cm_app_window();
  cm_camera3D_perspective(&sandbox->camera, (vec3){2, 2, 2}, 80.f,
                          (vec3){0, 0, 0}, window->r.w / (float)window->r.h);

  sandbox->shader = cm_shader_from_memory(
      &scene->gpu,
